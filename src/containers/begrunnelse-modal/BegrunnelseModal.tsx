...
const BegrunnelseModal = ({behandlingid, modalRef} : BegrunnelseModalProps) => {
    ...
    return (
        <Modal ref={modalRef} width="medium" className={styles.modal} onClose={() => {
            setError('');
            setBegrunnelse('');
        }}>
            <Modal.Body>
                <Select
                    ...
                    error={error}
                >
                    {<option key='ubesvart' value={''}>{"Velg begrunnelse"}</option>}
