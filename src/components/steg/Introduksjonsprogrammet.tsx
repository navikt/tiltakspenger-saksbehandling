...
import { useHentIntroduksjonsprogrammet } from '../../hooks/useHentIntroduksjonsprogrammet';
...
const Kvalifiseringsprogrammet = () => {
  ...
  const håndterLagreIntroSaksopplysning = (data: SkjemaFelter) => {
    ...
    fetch(`/api/behandling/${behandlingId}/vilkar/introduksjonsprogrammet`, {
      ...
      .then(() => {
        mutator(`/api/behandling/${behandlingId}/vilkar/introduksjonsprogrammet`);
      })
      .catch((error) => {
        throw new Error(
          `Ikke implementert enda: ${error.message}`,
        );
      });
  };
