...
export const avklarLesevisning = (
  ...
  const redigerbarTilstand =
    tilstand === 'opprettet' || tilstand === 'vilkårsvurdert';
  const erAdministrator =
  ...
  return {
    kanEndre:
      redigerbarTilstand &&
      innloggetSaksbehandler.navIdent === saksbehandlerPåSaken,
    knappAvbrytBehandling:
      redigerbarTilstand &&
      (innloggetSaksbehandler.navIdent === saksbehandlerPåSaken ||
        (erAdministrator && !!saksbehandlerPåSaken)),
    knappOppdater:
      (redigerbarTilstand || tilstand === 'tilBeslutter') &&
      innloggetSaksbehandler.navIdent === saksbehandlerPåSaken, //Om det også skal være i "tilBeslutter" avhengig av avklaring
    knappSendTilBeslutter:
      redigerbarTilstand &&
      innloggetSaksbehandler.navIdent === saksbehandlerPåSaken,
    knappGodkjennVis: tilstand === 'tilBeslutter' && kanBeslutte,
    knappGodkjennTillatt:
      tilstand === 'tilBeslutter' && girInnvilget && kanBeslutte, //Skal drift eller admin kunne "lage" avslag?
    knappSendTilbake:
      tilstand === 'tilBeslutter' &&
      (kanBeslutte || (erAdministrator && !!beslutterPåSaken)),
